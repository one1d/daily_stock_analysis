            ...
            if msg["role"] == "system":
                ...
                if msg.get("tool_calls"):
                    for tc in msg["tool_calls"]:
                        parts.append(
                            genai.protos.Part(
                                function_call=genai.protos.FunctionCall(name=tc["name"], args=tc["arguments"])
                            )
                        )
        ...
        for cm in chat_messages:
            contents.append(
                genai.protos.Content(
                    role=cm["role"], parts=[genai.protos.Part(text=p) if isinstance(p, str) else p for p in cm["parts"]]
                )
            )
